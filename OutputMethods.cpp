...
void StreamStore::store(const TrackerOutput& output) {
    ...
    ofstream svm_process;
    svm_process.open("svm testing",fstream::app);
    if(isTesting)
{
    svm_process<<output.gazepoint.x<<"  "<<output.gazepoint.y<<endl;
}
    stream.flush();
}
...
void SocketStore::store(const TrackerOutput& output) {
    ofstream outcome;
    outcome.open("Calibrating Output",fstream::app);
    ofstream outp;
    outp.open("Testing Output",fstream::app);
    if(isCalibrating){
    outcome<<output.gazepoint.x<<"  "<<output.gazepoint.y<<endl;}
    if(isTesting){
    outp<<output.gazepoint.x<<"  "<<output.gazepoint.y<<endl;}
