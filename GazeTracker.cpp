...
///Initualize updateSVM
void GazeTracker::updateSVM(void) {

    ofstream coord;
    coord.open("Coordinate",fstream::app);
    vector<double> xlabels(caltargets.size());
    vector<double> ylabels(caltargets.size());

    for(int i=0; i<caltargets.size(); i++) {
	xlabels[i] = caltargets[i].point.x/x_size;
	ylabels[i] = caltargets[i].point.y/y_size;
	coord<<"("<<xlabels[i]<<","<<ylabels[i]<<")"<<endl;
    }

    vector<SharedImage> images =
	getsubvector(caltargets, &CalTarget::image);
//	cout<<"This is getsubvector"<<"   "<<images<<endl;

    checkx=true;
    SVMx.reset(new ImProcess(images, xlabels,45,0,30));
    checkx=false;

    checky=true;
    SVMy.reset(new ImProcess(images, ylabels,65,90,17));
    checky=false;
...
///Initualize addExemplarSVM
void GazeTracker::addExemplarSVM(Point point,
			      const IplImage *eyefloat,
			      const IplImage *eyegrey)
{
    caltargets.push_back(CalTarget(point, eyefloat, eyegrey));
...
static void ignore_gt(const IplImage *) {}
...
///Change the output related to svm
void GazeTracker::update(const IplImage *image) {

//    cv::Mat Matrix(image);
//
//    ofstream imagefile;
//    imagefile.open("Input predicting",fstream::app);
//
//    if(isTesting)
//    imagefile<<Matrix<<endl;


    if (isActive()) {
	output.gazepoint = Point(SVMx->SVM_Predict(SharedImage(image, &ignore_gt),-2,30),
				 SVMy->SVM_Predict(SharedImage(image, &ignore_gt),101,17));
	output.targetid = getTargetId(output.gazepoint);
	output.target = getTarget(output.targetid);
    }
}
